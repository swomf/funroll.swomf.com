...
1. It's cheap on ram while supporting stuff like webp and jpg without hassle.
   - You'd be surprised how software like ristretto don't do so well here.
2. It's "minimal" but doesn't sacrifice usability.
   - View everything in a directory
   - Preserves transparency
   - etc.

My specific use case: I like having the entire background transparent,
i.e. the image simply floats. Therefore I use
[this patch ⇗](https://codeberg.org/nsxiv/nsxiv-extra/src/branch/master/patches/alpha),
with a small edit added, from nsxiv-extra.

<div class="ml-3">
  <span class="bright">Note:</span>
  The patch normally wants you to set "Nsxiv.window.alpha: 0.0" in
  ~/.Xresources then run something like
  <a href="https://wiki.archlinux.org/title/X_resources#Usage" target="_blank">xrdb -merge ~/.Xresources" ⇗</a> whenever X starts.

  Why would I want to add extra machinery when I can just poke the patch slightly?
  See the "edited by swomf" marker below.
</div>

```bash path=/etc/portage/package.use/nsxiv
media-gfx/nsxiv exif savedconfig
```

```bash path=/etc/portage/patches/media-gfx/nsxiv/alpha-275ec0d59.diff
